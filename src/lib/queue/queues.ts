 ...
 * BullMQ-safe id here so the two stay derivable from each other.
 */
...
export async function enqueueParse(campaignId: string): Promise<void> {
  await parseQueue.add("parse-campaign", { campaignId }, { jobId: toJobId(`parse:${campaignId}`) });
}
...
export async function enqueueIngest(campaignId: string): Promise<void> {
  await ingestQueue.add("ingest-campaign", { campaignId }, { jobId: toJobId(`ingest:${campaignId}`) });
}
...
export async function enqueueTranscribe(assetId: string): Promise<void> {
  await transcribeQueue.add("transcribe-asset", { assetId }, { jobId: toJobId(`transcribe:${assetId}`) });
}
...
export async function enqueueClip(assetId: string): Promise<void> {
  await clipQueue.add("clip-asset", { assetId }, { jobId: toJobId(`clip:${assetId}`) });
}
...
export async function enqueueRender(candidateId: string): Promise<void> {
  await renderQueue.add("render-candidate", { candidateId }, { jobId: toJobId(`render:${candidateId}`) });
}
...
export async function enqueueCompliance(renderedClipId: string): Promise<void> {
  await complianceQueue.add("evaluate-compliance", { renderedClipId }, { jobId: toJobId(`compliance:${renderedClipId}`) });
}
...
export async function enqueuePublish(input: {
  ...
  const jobId = toJobId(`publish:${input.renderedClipId}:${input.platform}:${input.mode}`);
...
export async function enqueueSubmission(publicationId: string): Promise<void> {
  await submitQueue.add("prepare-submission", { publicationId }, { jobId: toJobId(`submit:${publicationId}`) });
}
